...
import { Search, Database, Loader2, Check, X, Globe, Phone, Mail, MapPin, ExternalLink, Sparkles, Filter, ShieldCheck, AlertCircle, RefreshCw } from 'lucide-react';
  ...
  const startRealScraping = async () => {
    ...
    try {
      ...
      if (foundLeads.length === 0) {
        ...
        setScrapingStatus('Weryfikacja NIP w Białej Księdze...');
        
        // Automatyczna weryfikacja NIP dla każdego leada
        for (const lead of foundLeads) {
          if (lead.nip) {
            await checkWhiteList(lead.id, lead.nip);
          }
        }

        setScrapingProgress(100);
        setScrapingStatus('Scraping i weryfikacja zakończone sukcesem!');
      }
    } catch (error: any) {
          ...
          {isScraping && (
            <div className="space-y-3">
              <div className="flex justify-between text-sm font-bold">
                <span className={`text-sm ${scrapingStatus.startsWith('Błąd:') ? 'text-red-500' : 'text-slate-600'}`}>
                  {scrapingStatus}
                </span>
              ...
              <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                <motion.div 
                  ...
                  className={`h-full ${scrapingStatus.startsWith('Błąd:') ? 'bg-red-500' : 'bg-accent-blue'}`}
                />
              </div>
              {scrapingStatus.startsWith('Błąd:') && (
                <button 
                  onClick={startRealScraping}
                  className="mt-2 px-4 py-2 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg hover:bg-slate-800 transition-all flex items-center gap-2"
                >
                  <RefreshCw className="w-3 h-3" /> Spróbuj ponownie
                </button>
              )}
            </div>
          )}
